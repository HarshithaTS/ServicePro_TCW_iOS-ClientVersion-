...
- (void)viewDidLoad
{
    ...
    AppDelegate_iPhone *delegate = (AppDelegate_iPhone *) [[UIApplication sharedApplication] delegate];
    if (delegate.colleagueAction == @"ChangeColleague")
        self.title = @"Tasks for which Rep?";
    else if (delegate.colleagueAction == @"TransferColleague")
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ServiceManagementData *objServiceManagementData = [ServiceManagementData sharedManager];
    ...
    if (delegate.colleagueAction == @"TransferColleague")
    {
...
-(BOOL) transferColleagueTaskList
{
	...
	par3.Cdata = @"EVENT[.]SERVICE-DOX-TRANSFER[.]VERSION[.]0";	
	
	Z_GSSMWFM_HNDL_EVNTRQST00Service_ZgssmbssDatarcrd01 *par4 = [[Z_GSSMWFM_HNDL_EVNTRQST00Service_ZgssmbssDatarcrd01 alloc] init];	
	par4.Cdata = @"DATA-TYPE[.]ZGSXSMST_SRVCDCMNTTRNSFR21[.]OBJECT_ID[.]PROCESS_TYPE[.]SERVICE_EMPLOYEE";
    ...
    NSMutableArray *updateResponseMsgArray = [[NSMutableArray alloc] init];
			...
			{				
				...
				{					
					if([childNode2 stringValue]!=nil)
					{
						...
						{
							[updateResponseMsgArray addObject:[childNode2 stringValue]];
							localVar = TRUE; 
						}
                        
					/*	if([[getArrayAfterSplit objectAtIndex:0] isEqualToString:[objServiceManagementData.dataTypeArray objectAtIndex:12]])
						{
							
							NSString *tempFiledStr = @"";
							NSString *tempValueStr = @"";
							
							for(int i=0;i<[objServiceManagementData.ZGSCSMST_SRVCDCMNT01_COLLEAGUEFeildArray count];i++)
							{
								tempFiledStr = [tempFiledStr stringByAppendingString:[objServiceManagementData.ZGSCSMST_SRVCDCMNT01_COLLEAGUEFeildArray objectAtIndex:i]];								
								...
								if(i<([objServiceManagementData.ZGSCSMST_SRVCDCMNT01_COLLEAGUEFeildArray count]- 1))
								{
	...
	/*SELVAN*/
	//Retrieving those activity, Sparse, confirmations data's from Sqlite ...before making the arry is empty...
	//[objServiceManagementData.colleagueTaskListArray removeAllObjects];	
	//[objServiceManagementData fetchColleagueTaskList:[NSString stringWithFormat:@"SELECT * FROM '%@' WHERE 1",[objServiceManagementData.dataTypeArray objectAtIndex:11]]:-1];
	NSLog(@"response msg %@",updateResponseMsgArray);
